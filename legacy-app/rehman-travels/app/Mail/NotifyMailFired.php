...
class NotifyMailFired extends Mailable
{
        ...
        try {
            Mail::send('emails.'.$this->data['view'],$this->data['data'], function($message) {
                ...
                if($this->data['cc'] == true):
                    $message->bcc(env('MAIL_BC_ADDRESS'),env('MAIL_CC_TTILE'));
                    $message->bcc('info@rehmantravel.com','Rehman Travel');
                    $message->bcc('rt.webmaster15@gmail.com','Hamza Shah');
                endif;
                if($this->data['attach'] == true):
                    if(!empty($this->location) &&  File::isFile(storage_path("app/pdf/{$this->location}/{$this->itineraryRef}.pdf")) == true):
                        $message->attach(storage_path("app/pdf/{$this->location}/{$this->itineraryRef}.pdf"));
                    endif;
                endif;
