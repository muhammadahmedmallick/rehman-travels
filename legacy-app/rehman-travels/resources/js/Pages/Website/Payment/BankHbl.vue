...
<template>
    <LoadingBar v-if="loading"/>
        <input @click="cheapestFareFlightOrderPay" id="debit-hbl" type="radio" value="" name="list-credit" class="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2"/>
         Debit/Credit Card
</template>
...
export default {
    ...
    methods: {
        ...
        cheapestFareFlightOrderPay(){
            const cheapestFareFlight = JSON.parse(localStorage.getItem("cheapestFareFlight"))
            const passengerDetail = JSON.parse(localStorage.getItem("passengerDetails"))
            this.input.flightType = passengerDetail.params.ft
            this.input.email = passengerDetail.email
            this.input.mobileNo = passengerDetail.phoneNumber ?? passengerDetail.mobileNo
            this.input.totalAmount = passengerDetail.totalFare
            const persons = passengerDetail.persons ?? []
            this.input.firstName = persons.firstName[0] ?? 'Nouman'
            this.input.lastName = persons.lastName[0] ?? 'Khan'
            this.input.nationality = 'PK'
            ...
            if (this.errors.transactionType.includes('error') || this.errors.countryCode.includes('error')) {
                ...
                this.start();
                Service.doFetchRequest("/payonline/cheapest-fare-order-hbl-pay-online-request", "Create", this.input)
                    .then((response) => {
