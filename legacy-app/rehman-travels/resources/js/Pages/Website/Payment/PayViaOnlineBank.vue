...
<template>
    ...
    <input  @click="cheapestFareFlightOrderPay" id="bank" type="radio" value="card" name="list-credit" class="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2"/>
    Bank Transfer
</template>
...
export default {
    ...
    methods: {
        setInputs() {
            return {"paymentType":'card',"currencyCode":this.currencyCode,"airType":this.payInput.airType,"itineraryRef":this.payInput.itineraryRef,"payAmount":this.payTotalFare};
        },
        setErrors(errorType) {
        ...
        cheapestFareFlightOrderPay(){
            ...
            if (this.errors.paymentType.includes('error')) {
                ...
                this.start();
                Service.doFetchRequest("/viaonline/cheapest-fare-order-pay-via-online-bank-request", "Create", this.input)
                    .then((response) => {
