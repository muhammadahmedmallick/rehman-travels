    ...
    computed: {
        ...
        cheapestFareFlights: function () {
            if (this.cheapestFare.isRecordFounds.length !== 0) {
                ...
                const dedupedFlights = Array.from(
                    new Map(
                        availabilities
                            ...
                            .map(item => {
                                const carrier = item.flightAvailabilities?.price?.validatingCarrier || '';
                                const fare = item.totalFare;
                                const key = `${carrier}-${fare}`;
                                return [key, item];
                            })
                    ).values()
                ).map(flight => {
                    const leg1 = flight.flightAvailabilities?.legs?.leg1;
                    const seenDates = new Set();

                    if (leg1 && leg1.departureDate) {
                        const date = leg1.departureDate;
                        if (seenDates.has(date)) {
                            leg1.departureAirport = null;
                        } else {
                            seenDates.add(date);
                        }
                    }
                ...
                return _.orderBy(dedupedFlights, "totalFare", this.cheapestFare.isOrderType);
    ...
    methods: {
        ...
        airShopping: function () {
            ...
            this.providers.forEach((supplier) => {
                Service.doFetchRequest("/ticketing/cheapest-fare-airshopping-request", supplier, this.cheapestFare.payload)
                    .then((response) => {
                        if (response) {
        ...
        openFareRuleModal(cheapestFareFlight){
            ...
            if(cheapestFareFlight.flightAvailabilities.bundledServices !== undefined){
                ...
                Service.doFetchRequest("/ticketing/cheapest-fare-flight-fare-rule-request",'', payload)
                    .then((response) => {
                        if (response.length !== 0  && response.error !== undefined) {
                            this.cheapestFlightFareRule.loading = false;
                            this.cheapestFlightFareRule.responseError = `Internal Server Error ${response.error.code}`
                        }else if (response.length !== 0 && response.error === undefined) {
                            this.cheapestFlightFareRule.isRecordFounds = response
                            this.cheapestFlightFareRule.isWaitingResponse = ''
                            this.cheapestFlightFareRule.loading = false;
                        }
                    });
        ...
        sendMail() {
            ...
            if (this.cheapestMail.error.includes('error')) {
                ...
                Service.doFetchRequest("/ticketing/cheapest-fare-flight-fare-rule-send-pdf-email", 'Create', {'cheapestFlightFareRule': this.cheapestMail}).then(response => {
