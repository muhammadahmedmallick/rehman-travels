        ...
        <div class="">
            ...
            <div class="overflow-x-auto shadow-md">
                <table class="w-full md:w-[98%] text-sm text-left border-2 rtl:text-right text-gray-500 mx-auto mt-2 mb-2 rounded-full">
                    ...
                    <tbody>
                        <tr v-if="filterfranchise == ''" class="odd:bg-white even:bg-slate-50 border-b">
                            <td scope="row"
                                class="px-6 py-4 whitespace-nowrap col-span-12"
                                colSpan={6}> No Data Found</td>
                        </tr>
                        <tr v-else class="border-b odd:bg-white even:bg-slate-50" v-for="(franchise, franchiseKey) in filterfranchise"
                            :key="franchiseKey">
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchiseKey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchise.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchise.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchise.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (franchise.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (franchise.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchise.sequence }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (franchise.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ franchise.created_at }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(franchise, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(franchise, 'view')"><i
                            ...
                            <div class="flex flex-wrap -mx-3">
                                ...
                                <div class="w-full sm:w-1/2 md:w-1/4 lg:w-1/12 px-1 mb-5">
                                    ...
                                    <select
                                        ...
                                        <option v-for="(parent, parentKey) in $page.props.parentPages"
...
export default {
    ...
    data() {
        ...
        filterfranchise(){
            return this.$page.props.franchisePackages.filter(item =>{
    ...
    methods: {
        ...
        save: function () {
            if (this.input.id != "") {
                ...
                this.$inertia.post('/franchise/update-new-page', { 'formData': this.input }, {
                    onSuccess: (response) => {
