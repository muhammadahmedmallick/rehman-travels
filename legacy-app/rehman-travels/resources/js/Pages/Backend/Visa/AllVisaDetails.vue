    ...
    <AuthenticatedLayout>
        <div class="">
            <div class="flex justify-end m-3">
                ...
                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" :href="route('content.new-page')" class="bg-bgRGTBaseColor text-white p-2 rounded-md mr-3"><i
                        class="fa fa-plus"></i> &nbsp Add New Page </a>
                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" :href="route('visa.new-visa-packages')" class="bg-bgRGTBaseColor text-white p-2 rounded-md"><i
                        class="fa fa-plus"></i> &nbsp Add Visa Package </a>
            </div>
            <div class="overflow-x-auto shadow-md">
                <table class="w-full md:w-[98%] text-sm text-left border-2 rtl:text-right text-gray-500 mx-auto mt-2 mb-2 rounded-full">
                    ...
                    <tbody>
                        <tr v-if="filterVisas == ''" class="bg-white border-b">
                            <td scope="row"
                                class="px-6 py-4 text-center text-red-600 text-lg font-semibold whitespace-nowrap col-span-12"
                                colspan="12"> No Data Found.......</td>
                        </tr>
                        <tr v-else class="border-b odd:bg-white even:bg-slate-50" v-for="(getVisa, getVisakey) in filterVisas"
                            :key="getVisakey">
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisakey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisa.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisa.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisa.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getVisa.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getVisa.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisa.sequence }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getVisa.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getVisa.created_at }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(getVisa, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(getVisa, 'view')"><i
                                        class="fa fa-eye text-orange-400 cursor-pointer mr-3"></i></a>
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'owner'" @click="removeVisa(getVisa.id)"><i class="fa fa-trash text-red-600 cursor-pointer"></i></a>
                            </td>
                                ...
                                <div class="w-full sm:w-1/2 md:w-1/4 lg:w-1/12 px-1 mb-5">
                                    ...
                                    <select
                                        ...
                                        <option v-for="(parentPage, parentPageKey) in $page.props.parentPages"
                                            :key="parentPageKey" :value="parentPage['id']">{{ parentPage['title'] }}
                                        </option>
                                    </select>
                                </div>
...
export default {
    ...
    computed: {
        filterVisas(){
            return this.$page.props.visas.filter(item => {
    ...
    methods: {
        ...
        save: function () {
            if (this.input.id != "") {
                this.$inertia.post('update-visa-page', { 'formData': this.input }, {
                    onSuccess: (response) => {
                        this.$toast.success(this.input.packageTitle +' Updated Successfully.');
                        this.reset();
