        ...
        <div class="mx-auto justify-center items-center w-[80%]">
            ...
            <div class="w-full overflow-x-auto shadow-[0_35px_60px_-15px_rgba(50_50_105_0.15)] border-2 mt-4 md:overflow-hidden rounded-lg mb-2">
                <table class="table-auto w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tbody>
                        <tr v-if="filterAllUmrahs == ''" class="odd:bg-white even:bg-slate-50 border-b">
                            <td scope="row"
                                class="px-6 py-4 whitespace-nowrap col-span-12"
                                colSpan={6}> No Data Found</td>
                        </tr>
                        <tr v-else class="border-b odd:bg-white even:bg-slate-50" v-for="(getAllUmrah, getAllUmrahkey) in filterAllUmrahs"
                            :key="getAllUmrahkey">
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrahkey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrah.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrah.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrah.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ (getAllUmrah.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ (getAllUmrah.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrah.sequence }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ (getAllUmrah.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 text-xs">
                                {{ getAllUmrah.created_at }}</td>
                            <td scope="row" class="px-6 py-4 text-xs whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(getAllUmrah, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(getAllUmrah, 'view')"><i
...
export default {
    ...
    data() {
        ...
        filterAllUmrahs(){
            return this.$page.props.umrahPages.filter(item =>{
        ...
        save: function () {
            if (this.input.id != "") {
                ...
                this.$inertia.post('update-new-page', { 'formData': this.input }, {
                    onSuccess: (response) => {
                        this.$toast.success(this.input.packageTitle + ' is Updated Successfully.');
