    ...
    <AuthenticatedLayout>
        <div class="">
            <div class="flex justify-end m-3">
                ...
                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" :href="route('content.new-page')" class="bg-bgRGTBaseColor text-white p-2 rounded-md md:mr-3"><i
            ...
            <div class="overflow-x-auto shadow-md">
                <table class="w-full md:w-[98%] text-sm text-left border-2 rtl:text-right text-gray-500 mx-auto mt-2 mb-2 rounded-full">
                    ...
                    <tbody>
                        <tr v-if="filterHotels == ''" class="odd:bg-white even:bg-slate-50 border-b">
                            <td scope="row"
                                class="px-6 py-4 whitespace-nowrap col-span-12"
                                colSpan={6}> No Data Found</td>
                        </tr>
                        <tr v-else class="odd:bg-white even:bg-slate-50 border-b" v-for="(getHotel, getHotelkey) in filterHotels"
                            :key="getHotelkey">
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotelkey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotel.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotel.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotel.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getHotel.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getHotel.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotel.sequence }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (getHotel.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ getHotel.created_at }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(getHotel, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(getHotel, 'view')"><i
                            ...
                            <div class="flex flex-wrap -mx-3">
                                ...
                                <div class="w-full sm:w-1/2 md:w-1/4 lg:w-1/12 px-1 mb-5">
                                    ...
                                    <select
                                        ...
                                        <option v-for="(parent, parentIndex) in $page.props.parents"
                                            :key="parentIndex" :value="parent['id']">{{ parent['title'] }}
                                        </option>
                                    </select>
                                </div>
...
export default {
    ...
    methods: {
        concateUrlLink: function () {
            let parents = this.$page.props.parents;
            let packageTitle = this.input.packageTitle;
            if (this.input.packageTitle != "" && this.input.parentId != "") {
                for (let parent in parents) {
                    if (this.input.parentId == this.parents[parent].id) {
                        if(this.parents[parent].id == 12){
                            let parentTitle = (this.parents[parent].title).replace(/\s+/g, "").toLowerCase()
                            this.input.urlLink = (parentTitle + "/" + packageTitle).replace(/\s+/g, "-").toLowerCase();
                        }else{
                            this.input.urlLink = (this.parents[parent].title + "/" + packageTitle).replace(/\s+/g, "-").toLowerCase();
                        }
                        if (this.parents[parent].title == "World Tours") {
                            this.input.categories = "International Tour";
                        } else if (this.parents[parent].title == "Pakistan Tours") {
                            this.input.categories = "Domestic Tour";
                        } else {
                            this.input.categories = this.parents[parent].title;
                        }
                    }
                }
            } else {
