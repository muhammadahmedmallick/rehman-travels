        ...
        <div class="">
            ...
            <div class="overflow-x-auto shadow-md">
                <table class="w-full md:w-[98%] text-sm text-left border-2 rtl:text-right text-gray-500 mx-auto mt-2 mb-2 rounded-full">
                    ...
                    <tbody>
                        <tr v-if="filterWorldTours == ''" class="odd:bg-white even:bg-slate-50 border-b">
                            <td scope="row"
                        ...
                        <tr v-else class="odd:bg-white even:bg-slate-50 border-b"
                            v-for="(worldTour, worldTourKey) in filterWorldTours" :key="worldTourKey">
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTourKey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTour.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTour.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTour.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (worldTour.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (worldTour.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTour.sequence }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (worldTour.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ worldTour.created_at }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(worldTour, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(worldTour, 'view')"><i
                                        class="fa fa-eye text-orange-400 cursor-pointer mr-3"></i></a>
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'owner'" @click="removeVisa(getAllWorldTour.id)"><i
...
export default {
    ...
    computed: {
        filterWorldTours() {
            return this.$page.props.worldTours.filter(item => {
