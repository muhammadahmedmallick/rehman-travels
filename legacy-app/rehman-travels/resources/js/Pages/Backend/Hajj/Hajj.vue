    ...
    <AuthenticatedLayout>
        <div class="">
            <div class="flex justify-end m-3">
                ...
                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" :href="route('content.new-page')" class="bg-bgRGTBaseColor text-white p-2 rounded-md"><i
            ...
            <div class="overflow-x-auto shadow-md">
                <table class="w-full md:w-[98%] text-sm text-left border-2 rtl:text-right text-gray-500 mx-auto mt-2 mb-2 rounded-full">
                    ...
                    <tbody>
                        <tr v-if="filterHajjPackages == ''" class="odd:bg-white even:bg-slate-50 border-b">
                            <td scope="row"
                                class="px-6 py-4 whitespace-nowrap col-span-12"
                                colSpan={6}> No Data Found</td>
                        </tr>
                        <tr v-else class="border-b odd:bg-white even:bg-slate-50" v-for="(hajjPackage, hajjPackageKey) in filterHajjPackages"
                            :key="hajjPackageKey">
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackageKey + 1 }} </td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackage.packageTitle }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackage.parent_pages['title'] }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackage.urlLink }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (hajjPackage.type == 1 ? 'New' : 'Old') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (hajjPackage.showOnHome == 1 ? 'Yes' : 'No') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackage.sequence }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ (hajjPackage.status == 1 ? 'Active' : 'De-active') }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                {{ hajjPackage.created_at }}</td>
                            <td scope="row" class="px-6 py-4 whitespace-nowrap">
                                <a v-if="$page.props.auth.user.department == 1 || $page.props.auth.user.userType == 'agent' || $page.props.auth.user.userType == 'owner'" @click="edit(hajjPackage, 'edit')"><i
                                        class="fa fa-pen-to-square text-green-400 cursor-pointer mr-3"></i></a>
                                <a @click="edit(hajjPackage, 'view')"><i
                            ...
                            <div class="flex flex-wrap -mx-3">
                                ...
                                <div class="w-full sm:w-1/2 md:w-1/4 lg:w-1/12 px-1 mb-5">
                                    ...
                                    <select
                                        ...
                                        <option v-for="(parent, parentKey) in $page.props.parents"
                                            :key="parentKey" :value="parent['id']">{{ parent['title'] }}
                                        </option>
                                    </select>
                                </div>
...
export default {
    ...
    data() {
        ...
        filterHajjPackages(){
            return this.$page.props.hajjPackages.filter(item =>{
    ...
    methods: {
        concateUrlLink: function () {
            let parentPages = this.$page.props.parents;
        ...
        edit: function (umrahDetails, viewType) {
            this.isModalVisible = true;
            this.viewType = viewType;
            this.input = umrahDetails;
            this.input._method = 'PUT';
        },
        save: function () {
            if (this.input.id != "") {
                console.log(this.input.id);
                this.$inertia.post('hajj-dashboard/update-new-page', { 'formData': this.input }, {
                    onSuccess: (response) => {
